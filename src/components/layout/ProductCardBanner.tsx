...
const ProductCardBanner: React.FC = () => {
  const cards: CardProps[] = [
    ...
    },
    {
      ...
      subtitle: '', // Added empty string
      buttonText: '', // Added empty string
      content: 'Real Plant-Powered Protein Shake – Double Chocolate',
      hasImage: true,
    ...
    },
    {
      ...
      subtitle: '', // Added empty string
      buttonText: '', // Added empty string
      content: 'California Pizza Kitchen Margherita, Crispy Thin',
      hasImage: true,
      imageUrl: Margherita,
      additionalImageUrl: Coldsale,
      discount: 20,
  ...
  return (
    <div className=" flex-wrap xs:flex-col xs:items-center xs:justify-center grid grid-cols-1 md:grid-cols-4  lg:grid-cols-12 xs:px-8 p-4 mr-4 ml-4  xs:max-w-[470px] items-center justify-center xs:mx-auto sm:grid-cols-2 xs:gap-y-8">
  {cards.map((card, index) => {

     // Update column spans for SM screens
     const updatedColSpan = card.isBackgroundImage 
     ?  'sm:col-span-2 md:col-span-4' 
     : `sm:col-span-1 md:${card.colSpan}`;
    ...
    >
          ...
          <div className="relative flex items-center justify-center w-full xs:flex-col xs:items-center">
            ...
            {card.additionalImageUrl && (
              <img
                ...
                className="w-16 h-4 mt-40 lg:mt-44 object-cover rounded absolute left-4"
              />
            )} 
          </div>   
        )}
        ...
        {[0, 1, 3, 4, 5, 6, 8, 9].includes(index) && card.content && (
            <div className="font-medium text-base text-customBlack text-[24px] text-left w-full h-10 lg:h-16">
              {card.content}
            </div>
          )}
